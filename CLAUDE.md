...
- **Confinex está em transição para o Supabase**: localStorage + Google Sheets continuam como compatibilidade temporária; testes nomeados autenticados são gravados em `confinex_testes`, e negócios iniciados usam `confinex_avaliacoes` + `confinex_estimativas` (fotografia imutável). Consolidação previsto × realizado usa `confinex_consolidacoes` + `confinex_desvios`. Migração: `supabase/migrations/202607200001_confinex_avaliacoes.sql`.
