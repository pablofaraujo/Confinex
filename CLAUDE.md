...
- `painel-boi-gordo.html` lê `dados/painel-boi-gordo.json` sem cache sempre que abre e redesenha cartões, curva, manchetes e contexto. O workflow do repositório roda em dias úteis e sabe validar/commitar esse arquivo, mas preserva o último dado válido enquanto `PAINEL_BOI_GORDO_SOURCE_URL` não estiver configurada. Um run verde sem essa variável significa agenda executada, não cotação renovada; o aviso fica no resumo do workflow.
