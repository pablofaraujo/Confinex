...
| `fazenda-ametista.html` | Fazenda Ametista — ledger de entrada/saída de cabeças do rebanho próprio. KPI de estoque atual = soma de entradas − saídas. A seção interunidades lê vendas da Fazenda para o Confinamento sem misturar as apurações; permanece compatível enquanto a migração estiver pendente |
...
- **Consolidação privada possui staging próprio preparado**: `supabase/migrations/202608130001_staging_consolidacao_privada.sql` cria fontes idempotentes, negócios candidatos, versões, evidências, movimentos bancários em staging, conciliações, vínculos documentais candidatos e decisões imutáveis de revisão. A estrutura não importa dados e nunca promove automaticamente para `operacoes`, `compras`, `vendas`, `fluxo_caixa`, `transacoes_banco`, `gtas` ou notas fiscais. Usuários autenticados têm somente leitura; importação exige o executor com `service_role` e promoção operacional permanece em fluxo separado.
...
- A investigação das dúvidas pelo WhatsApp usa `tools/conciliar_whatsapp_pix.py`: pesquisa primeiro o valor do PIX nos históricos locais do Wey, reforça o candidato com data e contraparte e preserva valores repetidos como ambíguos. `wey-whatsapp-live-sync.service` mantém a captura contínua na VPS e o heartbeat tenta repará-la a cada cinco minutos. `tools/orquestrar_conciliacao_whatsapp.py`, executado diariamente pelo timer `wey-whatsapp-automation.timer`, abre uma janela exclusiva, verifica a cobertura por conversa, tenta backfill histórico serial e limitado, repete a conciliação e gera perguntas prontas nas pendências privadas; ao terminar, a captura contínua é retomada mesmo após falha. Nenhum desses componentes envia mensagens pelo WhatsApp, chama Supabase ou promove registros; buscas vazias com cobertura incompleta nunca são tratadas como inexistência.
- `tools/verificar_openclaw_canais.py`, executado a cada cinco minutos por `openclaw-agent-heartbeat.timer`, prova via probe somente leitura que Juan e Ceci estão autenticados no Telegram, Wey está conectado e saudável no WhatsApp, os grupos configurados continuam acessíveis e os vínculos/arquivos dos agentes permanecem íntegros. Falhas de processo reiniciam gateway, OCR, Chrome ou o reparador do `wacli` e são revalidadas antes de alertar. O heartbeat nunca envia mensagens aos grupos; estrutura ausente ou bot removido de um grupo exige alerta porque não pode ser recriado legitimamente pelo servidor.
