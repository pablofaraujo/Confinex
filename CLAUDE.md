...
- **Confinex está em transição para o Supabase**: localStorage + Google Sheets continuam como compatibilidade temporária; testes nomeados autenticados são gravados em `confinex_testes`. Agentes submetem negócios como `rascunho` pela RPC `submeter_negocio_confinex`; a fila em **Operações → Confinamento** revisa os indicadores e aprova pela RPC `aprovar_negocio_confinex`, mudando o status para `iniciado`. A estimativa original fica congelada em `confinex_estimativas`. Consolidação previsto × realizado usa `confinex_consolidacoes` + `confinex_desvios`. Migrações: `supabase/migrations/202607200001_confinex_avaliacoes.sql` e `supabase/migrations/202607200002_confinex_aprovacoes.sql`.
