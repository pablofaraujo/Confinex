...
- A idempotência persistente de compras está preparada em `supabase/migrations/202607250001_compras_idempotencia.sql`, mas não aplicada. Ela preserva compras antigas com chave nula, usa índice único parcial e permite ao cliente reconciliar timeout sem repetir cegamente uma gravação. O contrato e a ordem segura de implantação estão em `docs/idempotencia-compras.md`.
