...
- A conciliação documental local usa `tools/conciliar_documentos_operacionais.py` para ler exportações `.xlsx`/`.csv`/`.json`, ficha detalhada do IMA e OFX. `tools/consolidar_historico_telegram.py` acrescenta os exports HTML do Telegram, herda o autor em mensagens agrupadas, confere anexos por hash, exclui testes, preserva compras divergentes e cruza somente números exatos de GTA. Os resultados ficam em `docs/privado/` ou diretório temporário: correção posterior é apenas versão preferida, GTA exata é candidato forte, valor/data isolados não formam vínculo e não existe caminho de escrita ou chamada ao Supabase.
