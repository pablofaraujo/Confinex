...
- Após a aplicação da migração `202608210001_b3_referencias_sequenciais.sql`, cada negócio de bolsa recebe uma referência humana, persistente e imutável no formato `B3-AA-NNN`, como `B3-26-001`. A numeração é anual e atômica: duas gravações concorrentes não podem receber o mesmo código. A referência acompanha a posição aberta e o histórico e deve ser citada nas conversas com a mesa para permitir o cruzamento exato pelo Wey. O código identifica o negócio de bolsa; `CF-AA-NNN` continua identificando o lote físico atendido pelo hedge.
- A versão correspondente do Portfólio B3 mostra juntos o preço e a **data da entrada**. Data ausente aparece como pendência visível, não fica escondida somente na edição.
...
- Os indicadores gerais do BGI usam as posições deduplicadas, nunca a soma repetida da exposição por lote. **Cobertura cts bolsa** = vendas de hedge + termos − compras de hedge; especulação não cobre lotes. **Descoberto** = máximo entre contratos necessários − cobertura e zero; a exposição descoberta em arrobas multiplica esse saldo por 330. A marcação a mercado apresenta o resultado líquido em aberto, descontando corretagem e Finpec uma única vez. **Resultado realizado** soma `resultado_realizado` das posições encerradas, fechadas ou roladas pertencentes ao conjunto canônico do Portfólio B3 (`termo` iniciado por `bgp:`); registros históricos legados não entram novamente no consolidado. O valor deve coincidir com o Portfólio B3, enquanto créditos individuais por lote permanecem apenas no detalhamento da exposição.
...
- Na convenção internacional dos futuros, `V` é **outubro** e `X` é **novembro**. Assim, uma saída em outubro/2026 usa `BGIV26`; uma saída em novembro/2026 usa `BGIX26`. A interface mostra também o mês humano para evitar interpretar a letra isoladamente.
