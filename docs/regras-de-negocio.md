...
- Os indicadores gerais do BGI usam as posições abertas deduplicadas, nunca a soma repetida da exposição por lote. **Cobertura líquida aberta** = vendas de hedge + termos − compras de hedge; especulação não cobre lotes. **Descoberto** = máximo entre contratos necessários − cobertura líquida e zero; a exposição descoberta em arrobas multiplica esse saldo por 330. A marcação a mercado também usa a lista deduplicada e apresenta o resultado líquido em aberto, descontando corretagem e Finpec uma única vez.
