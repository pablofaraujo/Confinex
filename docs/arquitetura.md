...
- **idempotência de compras aplicada; executor pendente**: `supabase/migrations/202607250001_compras_idempotencia.sql` adicionou `compras.idempotency_key` nula e índice único parcial. A conferência antes/depois preservou compras, RLS, políticas e permissões. O cliente reconcilia timeout pela chave, retorna `duplicate` para os mesmos dados e rejeita a mesma chave com dados diferentes, mas ainda não foi implantado no VPS. O contrato e a homologação estão em [`docs/idempotencia-compras.md`](idempotencia-compras.md).
