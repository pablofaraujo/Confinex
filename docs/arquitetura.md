...
- **monitor fiscal proativo do AgroNota**: o skill da VPS consulta emitidas e recebidas, baixa XMLs idempotentemente e chama `tools/monitorar_agronota.py`. A GTA é lida de `infCpl`, `infAdFisco`, `xPed` ou `infAdProd`; múltiplas guias e vínculos ambíguos ficam em revisão. A natureza `natOp` contendo venda inicia sempre um novo negócio revisável e não procura operação anterior. O monitor pode enriquecer uma revisão ainda aberta quando um campo passa a ser reconhecido, mas preserva o evento anterior e cria uma nova auditoria. Seu cliente possui allowlist restrita ao staging fiscal, rascunhos, pendências e eventos; compra, venda, abate e pesagem não podem ser endereçados. O extrato bancário nunca é buscado: permanece pendente até ser enviado ao Juan.
