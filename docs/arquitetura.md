...
- **crm.html** — CRM de Gado: resumo de contatos, ofertas abertas/incompletas, negociações e próximas ações. Permite cadastrar uma oferta recebida, inclusive incompleta, sem criar compra, venda, lote ou comunicação externa.
- **financeiro.html** — visão autenticada de `fluxo_caixa`, `emprestimos`, `promissorias`, `transacoes_banco` e propostas do staging. Separa previsto/realizado e a pagar/receber, calcula saldos parciais, reúne vencimentos, dívidas e renegociações já representáveis nas fontes e deriva lembretes para 30 dias. A única decisão preparada usa uma RPC de lista fechada para confirmar ou rejeitar uma sugestão; ela não executa pagamentos, baixas, renegociações nem conciliação operacional.
...
- **fazenda-ametista.html** — ledger de entradas/saídas e consulta do último inventário físico por local/categoria. A tela mantém contagem física e saldo pelo histórico separados e lê vendas interunidades sem misturar as apurações.
...
- **prévia Wey→B3 (ciclo 1, desativada)**: `tools/planejar_atualizacao_b3.py` cruza offline snapshot privado e mensagens normalizadas para produzir somente pistas revisáveis. A ponte autoriza GET das duas tabelas; `alocacoes_hedge` usa no recorte seis campos reais (`id`, vínculos de posição/operação, quantidade, resultado creditado e criação), sem `updated_at`. A coleta foi homologada em memória com duas fotografias iguais do recorte declarado, prova não atômica nem global. O adaptador SQLite experimental e opt-in `tools/ler_cache_wey_b3.py` seleciona uma conversa privada exata e uma janela limitada com fuso explícito, normalizada para UTC, no cache do Wey, sempre com cobertura histórica parcial, e reutiliza o normalizador e o planejador sem criar fila. A leitura e a prévia foram homologadas em RAM com repetição determinística e origem de teste sintética; não produziram vínculo seguro. Marcador técnico de anexo exige igualdade exata e tipo de mídia compatível; texto humano e legenda válida são preservados, enquanto anexo sem texto gera somente contagem sanitizada, sem OCR. Sugestões por semelhança sem referência B3 canônica permanecem adiadas. O banco de entrada segue política exclusiva de fonte privada somente leitura; manifesto e relatório continuam recusados sob qualquer ancestral Git. O adaptador não integra runtime, modelo, mensagens, serviço ou executor e não é instalado na VPS por este ciclo. Nenhum teste de leitura ativa esses componentes. Não é migração ou autorização de escrita; uma futura mutação exigirá confirmação explícita, RPC transacional e recibo idempotente. O Portfólio externo preserva seu autosave serial/CAS já publicado, que não deve ser confundido com as escritas sequenciais do `bgi.html` legado.
...
- **recuperação textual Wey, opt-in e inativa**: `tools/recuperar_textos_mesa.py` preserva, em ordem determinística, o corpus humano limitado de uma conversa e janela explícitas. A autoria decorre somente do `from_me` estrito; termos informais e literais fornecidos são realces para revisão, nunca confirmação ou vínculo. O modo próprio do coletor ignora a ponte B3, dispensa origem Telegram e produz somente relatório privado. Ele não está ligado ao Juan, a hook, modelo, fila, serviço ou runtime.

- **adaptador Juan→mesa (preparado, não ativado)**: o mesmo hook de continuidade pode acionar `tools/recuperar_mesa_juan.py` como segunda fonte, sem novo serviço ou fila. O JavaScript faz apenas um pré-filtro fechado e repassa o envelope Telegram autenticado; o Python compara remetente, chat, tipo, tópico, conta e chaves de rota com uma única regra privada antes de abrir o cache. Alias e identidade são autorizações separadas: texto citado nunca libera acesso. A saída limita-se a 16 trechos e 24 kB, reserva metade da capacidade para mensagens mais recentes, inclui realces e vizinhas e mantém autoria, intervalos e omissões. Nada disso associa posição B3, consulta persistência, envia mensagem ou escreve. O patch v2 e a prova isolada de modelo são artefatos separados; implantação e ativação continuam pendentes.

- **bgi**: `v_exposicao_hedge`, `posicoes_hedge` (`referencia_bolsa` no formato humano `B3-AA-NNN`) + `referencias_bolsa_contadores` (contador anual interno e protegido) + `alocacoes_hedge` com `resultado_creditado` rateado; `cotacoes_bgi` (contrato ou `FISICO`; basis = físico − futuro). A migração `202608210001_b3_referencias_sequenciais.sql` foi aplicada em 21/08/2026. O Portfólio mostra a referência e a data de entrada sem exigir edição, e o Wey pode usar o código B3 como chave exata na leitura privada do WhatsApp.
...
- **CRM e ofertas (aplicado em 15/08/2026)**: `ofertas_gado` preserva inclusive ofertas incompletas e deduplica canal/conversa/mensagem; `negociacoes_gado` mantém o funil e o resultado; `interacoes_crm` forma o histórico por contato; `crm_followups` guarda próximas ações internas. Todas usam RLS, não concedem exclusão ou truncamento e não possuem gatilho de promoção operacional. `tools/ofertas_gado.py` é simulação por padrão, lista as cinco perguntas operacionais ausentes e só grava no CRM com `--executar` e identidade completa da mensagem; nunca envia WhatsApp ou Telegram.
...
- **Inventários da Fazenda**: `inventarios_fazenda` registra uma fotografia física por unidade, data, local e categoria. O peso total é gerado por cabeças × peso médio. A estrutura não altera `fazenda_ametista`, negócios ou interunidades; a view `v_inventarios_fazenda_resumo` entrega os totais da data. A ferramenta de registro é idempotente e bloqueia lotes parciais ou divergentes. Consulte [`docs/inventarios-fazenda.md`](inventarios-fazenda.md).
...
- **Conciliação corriqueira pelo WhatsApp**: o timer diário da VPS gera, em área privada, dúvidas para cada abate recente sem acerto final. O gerador consulta apenas `abates`, `acertos`, `operacoes`, `confinamentos`, `confinamento_contatos` e `contatos`; o orquestrador usa telefone ou nome para encontrar a conversa WACLI e pontua mensagens, nomes de anexos, quantidade, data, relatório, romaneio e acerto. O resultado é evidência candidata para revisão. A rotina não envia mensagens, não grava no Supabase e nunca marca recebimento ou encerra pendência apenas por semelhança textual.
...
- **autorreparo dos agentes e canais**: `tools/verificar_openclaw_canais.py` combina estado dos serviços, validação da configuração, vínculos dos agentes, consistência dos índices de sessão, probes autenticados de Juan/Ceci/WhatsApp, diretório dos grupos Telegram e uma leitura autenticada mínima do Supabase (DNS, TLS e REST). Referências de sessão sem arquivo são removidas somente com `sessions cleanup --fix-missing`, depois de uma prévia que recusa qualquer poda, limite ou remoção de artefato válido. Um probe oficial de modelos, limitado a um token e armazenado por trinta minutos em cache sanitizado, valida cada combinação distinta de primário e fallbacks; um XLSX mínimo descartável percorre o `arquivo_grupo_router.py` em `--dry-run`, sem sessão e sem Supabase. Como o sandbox do Juan não dispõe de DNS externo confiável, `ConfinexClient` encaminha consultas e escritas não operacionais de revisão por uma fila privada ao `juan-confinex-db-bridge.service`, supervisionado no host. O adaptador de `ConfinexClient` encaminha escritas somente para tabelas de revisão. A ponte geral mantém também ações legadas mutantes; não há bloqueio global de todas as tabelas operacionais. Ela repete somente leituras, nunca `POST` ou `PATCH`. O leitor de continuidade tem sua própria restrição exclusiva a `get_read`. A cada cinco minutos, falhas recuperáveis reiniciam somente o componente correspondente: gateway, trabalhador OCR, ponte Confinex, Chrome ou heartbeat do `wacli`; uma falha real de resolução no host reinicia `systemd-resolved`, sem acoplar o gateway a falhas do Confinex. Credenciais inválidas, modelos removidos e regressões do roteador são detectados e alertados, porém não disparam reinícios inúteis nem mutação automática de segredo/código. Depois todos os gates são repetidos. Nenhum teste ou reparo envia mensagem aos grupos. Um bot removido de grupo ou configuração estrutural perdida é detectado e alertado, mas não é recriado automaticamente.
- **resiliência do cliente Supabase**: leituras `GET` repetem somente erros transitórios de transporte, no máximo três vezes e com espera progressiva. Respostas HTTP não são mascaradas. Escritas e RPCs continuam com tentativa única para não duplicar efeitos; compras usam sua chave persistente para reconciliar resultados incertos.
...
- **monitor fiscal proativo do AgroNota**: o skill da VPS consulta emitidas e recebidas, baixa XMLs idempotentemente e chama `tools/monitorar_agronota.py`. Toda NF de venda emitida entra na indexação, sem significar automaticamente negócio novo. A GTA é lida de `infCpl`, `infAdFisco`, `xPed` ou `infAdProd`; GTA ou referência fiscal com uma única correspondência ligam o documento ao negócio existente, enquanto múltiplas correspondências permanecem ambíguas. A revisão classifica a nota como documento existente, complemento documental ou de animais, possível negócio novo ou relação ainda a conferir. O monitor pode enriquecer uma revisão aberta quando surge evidência, preservando a auditoria anterior. A varredura diária reconcilia a mesma janela de 30 dias usada no download, evitando que documentos anteriores à implantação do monitor fiquem fora da fila; as três execuções incrementais usam três dias. `--numero-nota` permite reprocessamento idempotente e estritamente direcionado de uma nota histórica, sem depender de `criado_em`. O heartbeat não consulta a API a cada cinco minutos: ele prova que os agendamentos, os executores e o log recente estão saudáveis. Seu cliente escreve apenas no staging fiscal, rascunhos, pendências e eventos; compra, venda, abate e pesagem não podem ser endereçados. O extrato bancário nunca é buscado: permanece pendente até ser enviado ao Juan.
...
- **staging da consolidação privada**: `supabase/migrations/202608130001_staging_consolidacao_privada.sql` separa importação, revisão e promoção. `fontes_importacao` identifica cada fonte por hash; `negocios_candidatos`, `negocio_versoes` e `evidencias_negocio` preservam consolidação e histórico; `transacoes_banco_staging` deduplica OFX por conta + FITID; `conciliacoes_candidatas` admite rateio muitos-para-muitos; `vinculos_documentais_candidatos` preserva GTA/NF sem associação automática; `decisoes_consolidacao` mantém o antes/depois imutável das revisões. `exportar_snapshot_consolidacao.py` e `auditar_staging_consolidacao.py` fornecem prova somente leitura por contagem e assinatura; `importar_ofx_staging.py` limita a entrada bancária às duas tabelas de staging; `materializar_revisoes_staging.py` cria uma única tripla determinística por candidato/grupo de duplicidade de alta prioridade, mantendo campos faltantes e versões distintas para decisão; `propor_conciliacoes_staging.py` cria apenas alocações pendentes em alvo único, sem atualizar a transação ou o alvo. Todas as tabelas têm RLS, leitura autenticada e escrita apenas pelo executor. Nenhuma confirmação nessa camada altera tabelas operacionais.
- **investigações proativas preparadas, não aplicadas**: a proposta `202608290001_investigacoes_revisao.sql` acrescenta um plano de controle sem substituir o staging. Adaptadores somente leitura produzem tarefas e referências a evidências; alternativas e pendências permanecem explicáveis. `tools/materializar_revisoes_staging.py` segue como única definição da tripla de revisão. No modo protegido, `materializar_revisao_investigada` cria a tripla, valida todos os membros/timestamps do grupo, vincula e anexa em uma transação; falha não deixa revisão parcial. O banco usa lease/fencing para resultados e travas compartilhadas para impedir promoção com investigação ativa. A preparação protegida é outra RPC atômica, `SECURITY DEFINER` exclusiva de `service_role`, com owner, `search_path` e ACL atestados; o navegador dependerá de um mediador autenticado no servidor e nunca poderá receber a credencial privilegiada. Sem o mediador, flag ativa falha antes de qualquer escrita e sem fallback legado. Evidência que chega depois de uma promoção terminal entra na outbox ativada por `202608290002`: `sem_gravacao` pode gerar sucessora do retrato selado, enquanto `com_gravacao` e corretiva stale exigem getter sanitizado, replanejamento explícito, plano novo e CAS. Revisão corretiva antiga é preservada, cancelada e auditada; nenhuma tabela operacional é reescrita. Nenhum planejador, consumidor, worker, timer, trigger de origem ou adaptador está ativo nesta fase. As migrações não foram aplicadas, a flag está desligada e nenhum dado foi escrito. Consulte [`docs/investigacoes-proativas.md`](investigacoes-proativas.md).
- **decisão bancária aplicada em 22/08/2026**: `supabase/migrations/202608220001_decisoes_conciliacao_financeira.sql` adicionou uma RPC `SECURITY DEFINER` executável somente por usuário autenticado. A função bloqueia usuário anônimo, decisão inválida, motivo vazio e conflito com decisão anterior; usa bloqueio de linha para concorrência e não duplica histórico quando a mesma decisão é repetida. A transação atualiza somente `conciliacoes_candidatas` e insere em `decisoes_consolidacao` e `eventos`. A verificação confirmou função ativa, `search_path=public`, acesso de `authenticated`, bloqueio de `anon` e assinaturas idênticas nas 20 tabelas auditadas.

- **sessão do WhatsApp**: antes de reparar, o heartbeat consulta `wacli doctor` em modo somente leitura. Falha transitória com sessão válida permite reinício; sessão ausente ou revogada bloqueia o reinício, gera `wacli_reautenticacao_necessaria` e aguarda novo pareamento legítimo. `wey-whatsapp-live-sync.service` não reinicia sozinho, evitando ciclo após resposta 401. O canal WhatsApp nativo do OpenClaw recebe proteção equivalente: quando o probe está indisponível e o gateway registra logout, `whatsapp_openclaw_reautenticacao_necessaria` evita reiniciar o gateway em ciclo. Uma falha independente do Telegram ainda pode reiniciá-lo.
...
A camada ativada em 05/09/2026, `recuperar_contexto_juan.py`, lê sessões locais do Juan
...
Extratos detalhados são priorizados para que respostas repetidas sobre a
ausência de registro não escondam os dados recebidos. A validação após backup
e reinício usou o ponto de entrada instalado e histórico privado, sem chamada
ao modelo ou envio ao grupo; confirmou também consultas GET pela ponte e
...
10. `.github/workflows/atualizar-painel-boi-gordo.yml` roda em dias úteis e consulta diretamente `DailyFluctuationHistory` da B3 para os doze vencimentos a partir do mês corrente. Somente respostas `OK`, com negócio positivo e pregão de até sete dias, podem atualizar `dados/painel-boi-gordo.json`; qualquer falha preserva o último arquivo válido e reprova o workflow. `atualizadoEmB3`, `referenciasAtualizadasEm` e `dataFonte` impedem que indicadores, físico, manchetes ou contexto antigos pareçam renovados junto com a curva. O ágio contra o físico permanece vazio até existir referência física contemporânea. CEPEA oferece API formal sob contratação; ela não foi contornada por raspagem frágil nem substituída por dado inventado.
