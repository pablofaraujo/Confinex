...
- **bases do simulador entre aparelhos**: `confinex_bases` mantém somente as premissas reutilizáveis de cada confinamento, separadas de estudos, avaliações e operações. A RLS restringe leitura e escrita ao usuário autenticado. Ao abrir o Confinex, o catálogo online é mesclado com o cache local sem substituir o estudo corrente; salvar, atualizar ou apagar uma base replica a mesma ação online. Bases antigas ficam locais até a ação explícita **Sincronizar bases**, evitando transmissão silenciosa. A migração aditiva é `supabase/migrations/202608120001_confinex_bases_online.sql` e não deve ser aplicada sem autorização.
