...
- **histórico privado do WhatsApp independente do Mac**: a VPS mantém um cache autenticado do `wacli`. `tools/orquestrar_conciliacao_whatsapp.py` executa a conciliação por valor, detecta cobertura insuficiente, agenda backfill serial e gera uma pendência com pergunta pronta quando a evidência não fecha. `infra/systemd/wey-whatsapp-automation.*` agenda o ciclo diário e substitui o timer simples de sincronização para evitar concorrência. O fluxo não envia mensagens, não altera conversas e não possui caminho para bases operacionais.
- **conciliação documental somente leitura**: `tools/conciliar_documentos_operacionais.py` cruza exportação fiscal do Agronotas, ficha detalhada do IMA, OFX e planilha de negócios. Identificadores iguais geram candidatos fortes; valor/data gera apenas candidato provável; duplicidades e ambiguidades permanecem sem vínculo. A ferramenta não possui `--executar`, não chama Supabase e guarda a saída sensível somente em `docs/privado/`. Consulte [`docs/consolidacao-fontes-operacionais.md`](consolidacao-fontes-operacionais.md).
