...
- Supabase: `confinex_testes` recebe versões nomeadas quando há sessão autenticada. Integrações externas e agentes usam `submeter_negocio_confinex`, que cria uma avaliação `rascunho` e congela a estimativa original. A fila em **Operações → Confinamento** (`confinamento.html`) mostra os indicadores e chama `aprovar_negocio_confinex` para mudar o negócio para `iniciado`; o simulador não aprova submissões externas. A ação direta **Iniciar negócio** continua disponível no simulador para uma decisão já confirmada. `confinex_consolidacoes` e `confinex_desvios` guardam realizado, desvios e comentários. Sheets permanece somente durante a transição.
