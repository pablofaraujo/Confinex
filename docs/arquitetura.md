...
- **conciliação documental somente leitura**: `tools/conciliar_documentos_operacionais.py` cruza uma ou mais exportações fiscais do Agronotas, ficha detalhada do IMA, um ou mais OFX e planilha de negócios. A combinação preserva históricos, deduplica documentos e lançamentos sobrepostos e distingue último documento da data final consultada. Identificadores iguais geram candidatos fortes; valor/data gera apenas candidato provável; duplicidades e ambiguidades permanecem sem vínculo. A ferramenta não possui `--executar`, não chama Supabase e guarda a saída sensível somente em `docs/privado/`. Consulte [`docs/consolidacao-fontes-operacionais.md`](consolidacao-fontes-operacionais.md).
