...
- **idempotência de compras preparada, não aplicada**: `supabase/migrations/202607250001_compras_idempotencia.sql` propõe `compras.idempotency_key` nula e índice único parcial. O cliente reconcilia timeout pela chave, retorna `duplicate` para os mesmos dados e rejeita a mesma chave com dados diferentes. Registros antigos continuam com chave nula e as políticas RLS não são alteradas. O contrato e a homologação estão em [`docs/idempotencia-compras.md`](idempotencia-compras.md).
...
Para compras, a proteção persistente contra repetição está preparada, mas só
pode ser implantada depois da migração explicitamente autorizada. A chave vem
da pendência confirmada, e timeout sem registro reconciliado nunca provoca novo
`POST` automático.
