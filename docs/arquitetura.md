...
- **Fazenda e Confinamento interunidades (estrutura preparada, não aplicada)**: `negocios_fazenda` registra a venda econômica, `fazenda_ametista` registra a saída física e `compras` registra a aquisição da operação de destino. `movimentacoes_interunidades` exige consistência entre as três faces antes da confirmação. `compras_componentes` detalha uma compra agregada sem duplicar seus totais; `operacao_participantes` mantém papéis explícitos e limita participações econômicas a 100%. Consulte [`docs/interunidades.md`](interunidades.md).
