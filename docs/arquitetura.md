...
- **prévia Wey→B3 (ciclo 1, desativada)**: `tools/planejar_atualizacao_b3.py` cruza offline snapshot privado e mensagens normalizadas para produzir somente pistas revisáveis. Não é adaptador ativo, executor, migração ou autorização de escrita; uma futura mutação exigirá confirmação explícita, RPC transacional e recibo idempotente. O Portfólio externo preserva seu autosave serial/CAS já publicado, que não deve ser confundido com as escritas sequenciais do `bgi.html` legado.
