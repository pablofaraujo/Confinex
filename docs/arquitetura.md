...
- `custoDinheiroOperacao` reúne compra, frete e parcelas do confinamento. Compra e frete ainda são componentes informativos de valor presente; `custoDinheiroConfinamento`, calculado pelos fluxos datados em `js/confinex-pagamento-confinamento.mjs`, reduz o lucro líquido uma vez. Adiantamento ou antecipação continuam como operações financeiras adicionais.
