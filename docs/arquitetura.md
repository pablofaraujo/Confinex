...
- **bb**: `operacoes` (`tipo_negocio='boi_balanca'`), `compras` (`prazo_dias`, `data_pagamento`, `pago`), `vendas` (`prazo_recebimento` = **data prevista** de recebimento, `recebido`, `funrural`, + `outros_custos`/`custos_obs` desde jul/2026), `negocios_boi_balanca` (desde jul/2026: previsão pré-lote — `status` previsto/confirmado/cancelado, campos do simulador + `operacao_id`/`confirmado_em` quando confirmado), `pendencias_documentos`, `contatos`. Pendências documentais aparecem somente em `aguardando_vendedor`; registros `recebido`, `dispensado`, `validado` ou `cancelado` permanecem no histórico, sem cobrança nas telas.
