...
- Supabase: `confinex_testes` recebe versões nomeadas quando há sessão autenticada. Integrações externas e agentes usam `submeter_negocio_confinex`, que cria uma avaliação `rascunho` e congela a estimativa original. A fila em **Operações → Confinamento** (`confinamento.html`) mostra os indicadores e permite aprovar pela RPC `aprovar_negocio_confinex` (`iniciado`) ou recusar pela RPC `recusar_negocio_confinex` (`cancelado`, preservado para auditoria); o simulador não decide submissões externas. A ação direta **Iniciar negócio** continua disponível no simulador para uma decisão já confirmada. `confinex_consolidacoes` e `confinex_desvios` guardam realizado, desvios e comentários. Sheets permanece somente durante a transição.
